...
public class PluginLoaderServiceForShopProcess extends Service {
	...
	public boolean stopService(Intent name) {
		if (null != name) {
			String dexKey = name.getStringExtra(PluginConstant.KEY_DEXPATH);
			pluginServiceDestroy(dexKey);
		}
		return false;
	}

	private void pluginServiceDestroy(String key) {
	...
	private void init(Intent intent) {
		...
		mDexClassLoader = PluginContextForShopProcess.getApkClassLoader(mDexpath, mDexoutputpath, mLibpath, null);
	}
		...
		try {
			if ((null != className) && (!"".equals(className))) {
				...
				if (null != serviceCache && serviceCache.containsKey(key)) {
					...
					methodInit.invoke(service, new Object[] { new PluginContextForShopProcess(getApplicationContext(), 0, mDexpath, pkgName, mDexClassLoader) });
