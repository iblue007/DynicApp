...
public abstract class PluginLoaderActivityBase extends Activity implements IActivityBase {
	...
	// public abstract void startPluginLoaderActivity(PluginLoaderActivityBase
	// pluginLoaderActivityBase, String mDexpath2, String pluginPackageName,
	// Intent intent, int requestCode, boolean forResult);
	//
	// public abstract ComponentName
	// startPluginLoaderService(PluginLoaderActivityBase
	// pluginLoaderActivityBase, String mDexpath2, String pluginPackageName,
	// Intent intent);
	//
	// public abstract boolean stopPluginService(PluginLoaderActivityBase
	// pluginLoaderActivityBase, String mDexpath2, String stringExtra, Intent
	// name);

	// public interface ComponentManager{
	// public void startPluginLoaderActivity(Context ctx, String path, String
	// pluginPackageName, Intent intent, int requestCode, boolean forResult);
	// public ComponentName startPluginLoaderService(Context ctx, String
	// pluginApkPath, String pluginPackageName, Intent intent);
	// public boolean stopPluginService(Context ctx, String dexpath, String
	// pluginPackageName, Intent name);
	// }
	...
	public void startActivity(Intent intent) {
		//插件中点击跳转到别的activity的时候，插件调用startActivity(new Intent(this,TestActivity.class));会跳到这个方法里面
	...
	public boolean onTouchEvent(MotionEvent event) {
		if (null != mTaskActivity) {
			try {
				Method methodOnTouchEvent = mTaskActivity.getCurrentClass().getDeclaredMethod("onTouchEvent", new Class[] { MotionEvent.class });
				methodOnTouchEvent.setAccessible(true);
				boolean result = (Boolean) methodOnTouchEvent.invoke(mTaskActivity.getCurrentActivity(), new Object[] { event });
